...
/* TASK 1: TO BE COMPLETED: (while L moved, added L in if and doWhile k changed by M N) */ 
stmt    : ';'
        | expr ';'      { emit(pop); }
        | IF '(' expr ')' M stmt
                        {  backpatch($5, pc - $5); }
...
expr    : ID   '=' expr { int place = 0; /*emit(dup); emit2(istore, $1->localvar);*/
        ...
        | expr GE  expr { emit3(if_icmplt, 7); emit(iconst_1); emit3(goto_, 4); emit(iconst_0); }
